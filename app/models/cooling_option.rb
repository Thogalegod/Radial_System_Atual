...
class CoolingOption < ApplicationRecord
  ...
  def check_for_associated_transformers
    if medium_voltage_transformers.any?
      errors.add(:base, "Não é possível excluir esta refrigeração pois está sendo usada por #{medium_voltage_transformers.count} transformador(es)")
      throw(:abort)
    end
  end
end
